...
MAPNAME = '/Users/sanjayk/Documents/Research/IRLTSH/consumable-irl/domains/ConsumableGridWorldMaps/11x11-TwoRooms.txt'  #Black and white image for a map
...
def selectStartGoalPoints(ax, img):
  ...
  while(occupied):
    ...
    print start,img[start[1]][start[0]]
...
def format(img):
  size = np.shape(img)
  imgn = np.zeros((size[0],size[1],3))
  for r in range(0,size[0]):
    for c in range(0,size[1]):
      if img[r,c] == 0:
         imgn[r,c,:] = [255.0,255.0,255.0]
      else:
         imgn[r,c,:] = [0.0,0.0,0.0]
         print r,c,"hi"
  return imgn

def main():
  print 'Loading map... with file \'', MAPNAME,'\''
  img = format(np.loadtxt(MAPNAME, dtype=np.uint8))
  print img
  fig = ppl.gcf()
  fig.clf()
